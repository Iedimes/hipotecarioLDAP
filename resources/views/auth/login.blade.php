						...
						<auth-form
							...
							<form class="form-horizontal" role="form" method="POST" action="{{ url('/login') }}" novalidate>
								...
								<div class="auth-header">
									<h1 class="auth-title">{{ trans('LOGUEO PARA LDAP') }}</h1>
								...
								<div class="auth-body">
									...
									<div class="form-group" :class="{'has-danger': errors.has('email'), 'has-success': fields.email && fields.email.valid }">
										...
										<div class="input-group input-group--custom">
											...
											<input type="text" v-model="form.email" class="form-control" :class="{'form-control-danger': errors.has('email'), 'form-control-success': fields.email && fields.email.valid}" id="username" name="username" placeholder="{{ trans('brackets/admin-auth::admin.auth_global.email') }}">
										</div>
